...
import { faGift, faCompass, faMountain, faLeaf, faLocationDot, faRoute, faChevronDown, faWater, faHiking, faCity, faLandmark, faTree, faUmbrellaBeach } from "@fortawesome/free-solid-svg-icons";
const Trips = () => {
    ...
    const fetchTrips = async () => {
      try {
        ...
        const data = response.data.map((trip) => {
          const image = trip.trip_img?.split(",")[0];
          const imageUrl = image
            ? `http://localhost:5000/uploads/trips/${image}`
          ...
          return { ...trip, trip_img: imageUrl, normalized_type: normalizedType };
        });
        ...
        const uniqueTripTypes = [...new Set(data.map(trip => trip.trip_type?.trim() || "other"))];
    ...
    <div className={styles.tripsPage}>
      ...
      <div className={styles.hero}>
        ...
        <div className={styles.heroContent}>
          <div className={styles.circleFilters}>
            <button
              className={`${styles.circleFilter} ${activeType === "all" || !activeType ? styles.active : ""} ${previousType === "all" ? styles.returning : ""}`}
              onClick={() => {
            ...
            <button
              className={`${styles.circleFilter} ${activeType === "טבע" ? styles.active : ""} ${previousType === "טבע" ? styles.returning : ""}`}
              onClick={() => {
                setPreviousType(activeType);
                setActiveType("טבע");
              }}
            ...
            <button
              className={`${styles.circleFilter} ${activeType === "מים" ? styles.active : ""} ${previousType === "מים" ? styles.returning : ""}`}
              onClick={() => {
                setPreviousType(activeType);
                setActiveType("מים");
              }}
            ...
            <button
              className={`${styles.circleFilter} ${activeType === "טבע ומים" ? styles.active : ""} ${previousType === "טבע ומים" ? styles.returning : ""}`}
              onClick={() => {
                setPreviousType(activeType);
              ...
              data-position="6"
            >
              <FontAwesomeIcon icon={faUmbrellaBeach} className={styles.filterIcon} />
              <span>טבע ומים</span>
            </button>
            
            <button
              className={`${styles.circleFilter} ${activeType === "אתרים ארכיאולוגיים" ? styles.active : ""} ${previousType === "אתרים ארכיאולוגיים" ? styles.returning : ""}`}
              onClick={() => {
                setPreviousType(activeType);
              ...
              data-position="8"
            >
              <FontAwesomeIcon icon={faLandmark} className={styles.filterIcon} />
              <span>אתרים ארכיאולוגיים</span>
            </button>
            
            <button
              className={`${styles.circleFilter} ${activeType === "מסלולים בעיר" ? styles.active : ""} ${previousType === "מסלולים בעיר" ? styles.returning : ""}`}
              onClick={() => {
                setPreviousType(activeType);
            ...
            <button
              className={`${styles.circleFilter} ${activeType === "הליכה" ? styles.active : ""} ${previousType === "הליכה" ? styles.returning : ""}`}
              onClick={() => {
                setPreviousType(activeType);
                setActiveType("הליכה");
              }}
      ...
      <div className={styles.gridContainer} id="trips-grid">
        ...
        {activeType && (
          <div className={styles.selectedTypeTitle}>
            <h2>
              {activeType === "all" ? "כל הטיולים" : activeType}
            </h2>
            ...
            <div className={styles.grid}>
              {getDisplayedTrips().map((trip, i) => (
                <div
                  ...
                  key={trip._id || `trip-${i}`}
                  onClick={() => navigate(`/trip/${trip._id || trip.trip_id}`)}
                >
                  <TripsCard {...trip} />
                </div>
              ))}
      ...
      <div className={styles.surpriseSection} id="surprise-trip">
        ...
        <p className={styles.surpriseDescription}>
          Can't decide where to go? Let us surprise you with a handpicked destination that matches your sense of adventure.
        </p>
